!(function($){
	$(function(){
		var $woocommerce = $('.woocommerce.ro-products-grid');
		$woocommerce.each(function(){
			var $this = $(this),
				$products = $this.find('.ro-products'),
			...
			$pagination.on('click', 'a', function(e){
				...
				$.ajax({
					...
					data: { action: 'ro_render_products_grid', paged: paged_num, params: $params},
					success: function(data){
						var obj = JSON.parse(data);
						$products.removeClass('ajax-loading');
						
						$products.find('.ro-product-items').html(obj.products_content);
